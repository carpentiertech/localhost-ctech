...
window.masterVolume = audioContext.createGainNode(context, context.destination, 0);
//document.querySelector('#volume').addEventListener('change', function(event){
  //masterVolume.gain.value = 0;}); //event.target.value;
...
//audioContext.getAudioByXhr('https://cdn.rawgit.com/nigeljohnwade/AudioContext/master/audio/Inchdown.wav', window.reverbUnit.convolver, function(){
//  document.querySelector('h1').innerText += ' (loaded)';
//});

//Set up canvas element
document.querySelector('#oscilliscopeVisualiser').width = document.querySelector('#oscilliscope').width;
document.querySelector('#oscilliscopeVisualiser').height = document.querySelector('#oscilliscope').width/8;

//Master section events
//document.querySelector('#volume').addEventListener('change', function(event){
//  masterVolume.gain.value = 0; //event.target.value;
//});
...
animateDisplay();
	...
	<script type='text/javascript'>
      ...
      canvasCtx.canvas.height = window.innerWidth/8;
      window.drawWaveform = function(){
			  analyser.getByteTimeDomainData(dataArray);
			  canvasCtx.fillStyle = 'transparent';
			  canvasCtx.lineWidth = 1;
